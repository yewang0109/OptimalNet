# Simulation for strategy evolution on networks of size N=5
# Calculate fixation probabilities across 21 graphs
# N: Network size
# G: Number of generations
# n_avg: Number of independent runs
...
def f_death_birth_process(strategy, matrix, neighbor_record, number_record, t_array, c_array, p_array, R, num):
    ...
    for index in neighbor_of_death:
        fitness_array[tik] = matrix[death, index] * np.exp(delta * f_cal_payoff(index, matrix, strategy, t_array, c_array, p_array, R, num))
        tik += 1
...
def f_single_turn_cooperator(neighbor_record, number_record, mat, t_array, c_array, p_array, R, num):
    ...
    tik1 = 0
    while tik1 < G:
        strategy = f_death_birth_process(strategy, mat, neighbor_record, number_record, t_array, c_array, p_array, R, num)
...
def f_avg_return_cooperator(neighbor_record, number_record, static_adj_mat, n_avg, t_array, c_array, p_array, R, num):
    ...
    for i in range(n_avg):
        val += f_single_turn_cooperator(neighbor_record, number_record, static_adj_mat, t_array, c_array, p_array, R, num)
    return val / n_avg
...
def main(t_array, c_array, p_array, R, Gp):
    ...
    result_list_cooperator = list(pool.map(f_avg_return_cooperator, neighbor_record_list, number_record_list, static_adj_mat_list, n_avg_list, t_array_list, c_array_list, p_array_list, R_list, num_list))
    result_list_defector = list(pool.map(f_avg_return_defector, neighbor_record_list, number_record_list, static_adj_mat_list, n_avg_list, t_array_list, c_array_list, p_array_list, R_list, num_list))
...
if __name__ == "__main__":
    ...
    simu_arr_cooperator = np.zeros(21)
    simu_arr_defector = np.zeros(21)

    data = scio.loadmat('n_5.mat')
    for i in range(1, 22):
        var_name = f'graph_{i}'
        if var_name in data:
            A = data[var_name]
        ...
        simu_arr_cooperator[i-1] = np.mean(result_list_cooperator)
        simu_arr_defector[i-1] = np.mean(result_list_defector)

    print(simu_arr_cooperator, simu_arr_defector)

    scio.savemat("n_5_couple_effect.mat", {
        'rhoc_array_n_5': simu_arr_cooperator,
        'rhod_array_n_5': simu_arr_defector
    })
